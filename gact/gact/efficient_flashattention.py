...
class FlashAttentionController():
...
class EfficientMemoryFlashAttention(MistralFlashAttention2):
    def __init__(self, config, layer_idx, compress_type: str = "JPEG", compress_quality: int = 50, quantization_shape: int = 64):
        super(EfficientMemoryFlashAttention, self).__init__(config, layer_idx)
        self.compress_type = compress_type
        ...
        self.quantization_shape = quantization_shape
        self.controller = FlashAttentionController(compress_type, quantization_shape, self.jpeg_processor, self.dct_processor)
